...
| 44 | 评审方卡在信任框【故障演练】【跳过】 | d | 真 | 5′ | 在场 | 30 |
| 50 | 第二个项目并行 | f e | 真，4 个 | 25′ | 人看 | 01 10 30 |
| 60 | 协议兼容【故障演练】【白盒】【跳过】 | g | 真 | 8′ | 中途接入一次 | 01 |
| 61 | 事件文件长期增长【故障演练】【白盒】【跳过】 | g | 真 | 5′ | 跑脚本 | 01 |
| 70 | 退出整个终端软件【跳过】 | g | 真 | 10′ | 在场 | 30 |
| 71 | 过夜【跳过】 | g | 真 | 跨夜 | 早晚各 10′ | 70 |
| 99 | 清理与核对 | — | — | 5′ | 在场 | — |

覆盖：a 临时委派；b 长期驻留的工作 agent；c 文件交接的请求 / 交付循环；d 异常路径；e 观察；f 多项目并存；g 生命周期与故障；h 内容。

2026-09-16 决定：44、60、61、70、71 跳过（白盒或极端场景，日常用不到），本轮只补做 50 和 99。
44 的 prepare 已经跑过，留下的 `/tmp/clab/untrusted-…` 由 99 里删 `/tmp/clab` 那一步带走，不用单独清。
...
- 结果：☑ 跳过　☐ 通过　☐ 不通过
- 备注：2026-09-16 决定跳过——极端/白盒：构造一个从没信任过的仓库让 B 卡在信任框，日常用不到。
...
- attach 旧栏位：
- 结果：☑ 跳过　☐ 通过　☐ 不通过
- 备注：2026-09-16 决定跳过——白盒：改 corral 代码造旧栏位/协议不兼容，日常用不到。

## 61 事件文件长期增长【故障演练】【白盒】【跳过】
...
- 结果：☑ 跳过　☐ 通过　☐ 不通过
- 备注：2026-09-16 决定跳过——白盒：人工追加 50MB 事件文件，日常用不到。
...
- dev 的回答：
- 结果：☑ 跳过　☐ 通过　☐ 不通过
- 备注：2026-09-16 决定跳过——极端：退出整个终端软件，今天不做。
...
- 栏位内存变化：　　　事件文件变化：
- 结果：☑ 跳过　☐ 通过　☐ 不通过
- 备注：2026-09-16 决定跳过——极端且跨夜：与今天收尾冲突。
